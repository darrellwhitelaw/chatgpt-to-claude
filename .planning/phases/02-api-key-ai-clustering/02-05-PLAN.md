...
<task type="auto">
  <name>Task 2: Create ClusteringView, update useCluster hook, wire App.tsx</name>
  <files>
    src/screens/ClusteringView.tsx
    src/hooks/useCluster.ts
    src/App.tsx
    ...
    onEvent.onmessage = (msg) => {
      switch (msg.event) {
        ...
        case 'batchSubmitted':
          ...
          break;
        case 'polling':
          // Update elapsed time via a separate field or just keep stage label
          useAppStore.setState({ stage: 'Clustering conversations...' });
          // Store elapsed for ClusteringView display
          useAppStore.setState((s) => ({
            ...s,
            _elapsedSecs: msg.data.elapsedSecs,
          } as typeof s));
          break;
        case 'complete':
...
NOTE: The `_elapsedSecs` approach is a quick hack. A cleaner alternative is to use `useState` in ClusteringView's parent and pass elapsed via props. Since ClusteringView reads from the store's `stage` prop, add a simple `elapsedSecs: number` field to AppState in appStore.ts — add it as `elapsedSecs: number` (default 0) alongside the other Phase 2 fields. Then the polling event handler sets it. Update appStore.ts to include this field and include it in `reset()`.

**Update appStore.ts** — add `elapsedSecs: number` field and initialize to 0, clear in reset.
...
Destructure `clusterError` from `useAppStore()` in App.
  ...
  <verify>
    ...
    <manual>Confirm ClusteringView.tsx in src/screens/. Confirm useCluster.ts exports both fetchCostEstimate and startClustering. Confirm CostScreen has onProceed prop. Confirm tsc passes.</manual>
  </verify>
  <done>
    `npx tsc --noEmit` passes. ClusteringView renders spinner + stage + elapsed time. useCluster handles all ClusterEvent variants. CostScreen's Proceed calls startClustering via onProceed prop. Error screen has "Try again" returning to cost-ready.
  </done>
</task>
